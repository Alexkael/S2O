...
def eval_adv_test_whitebox(model, device, test_loader, attack_method):
    ...
    for data, target in test_loader:
        ...
        err_natural, fgsm_err_robust = _fgsm_whitebox(model, X, y)
        err_natural, pgd_err_robust = _pgd_whitebox(model, X, y)
        err_natural, cw_err_robust = _cw_whitebox(model, X, y)
        '''
    ...
    print('natural_acc_total: ', 1-natural_err_total/10000)
    ...
    if args.white_box_attack:
        ...
        for epoch in range(100,200):
            ...
            if os.path.exists(path):
                ...
                model = nn.DataParallel(getattr(models, 'WideResNet')(num_classes=10)).to(device)
